...
class TargetSpace(object):
    ...
    def observe_point(self, x):
        ...
        self.add_observation(x, xx, f)

        return f

    # % ADD POINT
    def add_observation(self, x, xx, f):
        """
    ...
    def _allocate(self, num):
        ...
        #  Allocate new memory
        _Xnew = np.empty(num, dtype=object)
        _XXnew = np.empty((num, self.nfeatures))
        ...
        if self._X is not None:
            ...
            _XXnew[:self.length] = self._XX[:self.length]
