...
from .my_space import TargetSpace
...
class MOBayesianOpt(object):
    ...
    def __init__(self, target, NObj,  constraints=[],
        ...
        self.ga = random_points
        self.individual_class = random_points[0]
        self.random_points_generator = random_points[1]
        self.crossover = random_points[2]
        self.mutation = random_points[3]
        ...
        self.space = TargetSpace(self.target, self.NObj,
    ...
    def initialize(self, init_points=None, Points=None, Y=None):
        ...
        if init_points is not None:
            ...
            rand_points = [self.random_points_generator(self.individual_class) for _ in range(init_points)]
    ...
    def maximize(self,
        ...
        for i in range(n_iter):
            ...
            for i in range(self.NObj):
                yy = self.space.f[:, i]
                self.GP[i].fit(self.space.xx, yy)
            ...
            if self.space.RS.uniform() < self.NewProb:
                ...
                self.x_try = self.mutation(self.x_try)
    ...
    def __LargestOfLeast(self, front, F):
        print("-->", type(front[0]))
        if type(front[0]) is not np.ndarray:
            print("---> that's it")
            front = [ self.get_features(x.body) for x in front ]
            F = [ self.get_features(x.body) for x in F ]
...
#    @staticmethod
    def __MinimalDistance(self, X, Y):
        N = len(X)
        Npts = len(Y)
        DistMin = float('inf')
        for i in range(Npts):
    ...
    def __ObjectiveGP(self, individual):
        ...
        xx = self.get_features(individual.body).reshape(1, -1) # reshape because one sample
